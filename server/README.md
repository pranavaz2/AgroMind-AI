...
`AI_SERVICE_URL` points to the FastAPI microservice. The Node backend calls
`POST /api/v1/predictions/leaf-disease` with Axios and forwards the uploaded
image as multipart form data. `AI_SERVICE_TIMEOUT_MS` prevents crop scan
requests from hanging forever if the Python service is slow or offline.

## Node To Python AI Communication

AgroMind uses two backend services:

- The Node.js API owns authentication, users, farms, scan history, image storage,
  notifications, and the mobile-facing API contract.
- The Python FastAPI service owns TensorFlow model loading, image preprocessing,
  model inference, and disease prediction.

This is a microservices setup: each service has one focused responsibility and
communicates over HTTP instead of importing each other's code.

Request flow for a crop scan:

1. The mobile app sends `POST /api/v1/scans` to Node with an `image` file.
2. Multer validates the upload and keeps the image in memory as `req.file.buffer`.
3. `scanService` uploads the image to Cloudinary for permanent storage.
4. `aiService` creates a multipart `FormData` payload and sends it to FastAPI
   with Axios.
5. FastAPI preprocesses the image, runs the TensorFlow model, and returns JSON
   with disease name, confidence score, severity, and treatment suggestion.
6. Node normalizes the AI response into the scan format used by the mobile app.
7. Node saves the scan and AI summary in PostgreSQL through Prisma.

The integration code lives in `src/services/aiService.js`. Keeping it behind
`analyzeLeafImage(imageBuffer, mimeType)` means controllers and scan workflow
code do not need to know whether the prediction came from TensorFlow, Gemini, or
a fallback path.

## Scalability Benefits

This split gives AgroMind room to grow:

- Scale Node horizontally for normal app traffic.
- Scale Python separately for CPU/GPU-heavy inference.
- Deploy the AI service on a GPU machine without moving auth or database code.
- Update or retrain TensorFlow models without redeploying the whole backend.
- Keep failures isolated: if Python is unavailable, `AI_PROVIDER=auto` can fall
  back to Gemini, while `AI_PROVIDER=python` can fail scans clearly.
