...
export default function ScanScreen({ navigation }) {
  ...
  async function handleScan() {
    ...
    setIsAnalyzing(true);
    try {
      const result = await analyzeCrop(selectedImage.uri);
...
const styles = StyleSheet.create({
  ...
  emptyIconRing: {
    ...
    // Note: borderStyle 'dashed' is NOT supported on Android Views.
    // We use a thin solid border instead for cross-platform safety.
  },
  emptyIconInner: {
    width: 60,
    height: 60,
